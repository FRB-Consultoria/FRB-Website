...
// Preferências de notificação por e-mail (toggle por usuário)
...
// ── Permission list (grade de cards, 2 colunas) ──────────────────────────────
...
export const EditClientModal = ({ user, client_id }) => {
  const { updateUser, patchUser } = useContext(AdminContext);

  const [perms, setPerms] = useState(() => ({
    perm_bi:                 !!user.perm_bi,
    perm_powerbi:            !!user.perm_powerbi,
    perm_faturamento:        !!user.perm_faturamento,
    perm_faturamento_admin:  !!user.perm_faturamento_admin,
    perm_benefits:           !!user.perm_benefits,
    perm_benefits_billing:   !!user.perm_benefits_billing,
    perm_benefits_dashboard: !!user.perm_benefits_dashboard,
    perm_admin:              !!user.perm_admin,
    perm_book:               !!user.perm_book,
    perm_dash_evolucao:      !!user.perm_dash_evolucao,
    perm_dash_divergencias:  !!user.perm_dash_divergencias,
    notify_health:           !!user.notify_health,
  ...
  const toggle = (key) => {
    ...
    // perm_faturamento e perm_faturamento_admin são mutuamente exclusivos:
    // ativar um desativa o outro automaticamente.
    const patch = { [key]: next };
    const stateUpdate = { [key]: next };

    if (next) {
      if (key === "perm_faturamento") {
        patch.perm_faturamento_admin = false;
        stateUpdate.perm_faturamento_admin = false;
      } else if (key === "perm_faturamento_admin") {
        patch.perm_faturamento = false;
        stateUpdate.perm_faturamento = false;
      } else if (key === "perm_benefits_billing") {
        // Admin bate-conferência já tem dashboard embutido · desativa o perm separado
        patch.perm_benefits_dashboard = false;
        stateUpdate.perm_benefits_dashboard = false;
      } else if (key === "perm_benefits_dashboard") {
        // Viewer-only não pode ser processador ao mesmo tempo
        patch.perm_benefits_billing = false;
        stateUpdate.perm_benefits_billing = false;
      }
    }

    setPerms((p) => ({ ...p, ...stateUpdate }));
    patchUser(user.id, patch, client_id);
  };
        ...
        <div>
          <form
            onSubmit={handleSubmit((body) =>
              updateUser(body, user.id, client_id, "Atualizar")
            )}
          >
            <FormGrid>
              <Field>
              ...
              <ActionsRow>
                <SaveBtn type="submit">Atualizar</SaveBtn>
              </ActionsRow>
            </FormGrid>
